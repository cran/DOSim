...
This vignette demonstrates how to use the \Rpackage{DOSim} package easily. \Rpackage{DOSim} is used to calculate DO terms similarity and genes similarity, and meanwhile it provides functions to extract information about Disease Ontology (DO) (e.g. visualizing terms hierachies) and conduct DO Enrichment analysis. 

To use \Rpackage{DOSim} package, start with the following codes below:
<<results=hide>>=
library(DOSim)
help(DOSim)
@ 
...
\section{Measuring the Similarity between DO Terms}Terms in DO are organized in Directed Acyclic Graph (DAG). Previous studies have developed many methods to calculate terms similarities. \Rpackage{DOSim} implements two types of approaches for the calculation of similarity between terms in DO. One is node-based, in which the calculation of the similarity between DO terms is based on the attributes of nodes (node properties); the other is edge-based, which uses the attributes of edges between nodes (edge types) as the measure. In node-based approaches, information content ($IC$) is used to quantify the specificity and information of a DO term. In our work, the $IC$ of a DO term $t$ is defined as follows:
\begin{equation}\label{eq:ic}
IC(t)=-\log p(t)
\end{equation}

In total, \Rpackage{DOSim} implementes thirteen different methods to calculate the similarity between DO terms, of which ten are node-based, and three are edge-based. The function \textit{getTermSim} is the interface for users to calculate DO terms similarity. 
...
In the following text, we will introduce all these thirteen methods in an individual sub-section, which is named after the parameter name for "method" in function \textit{getTermSim}.
...
Compared with Resnik's method, Jiang and Conrath's method scales the information content of the MICA by the information content of the individual terms \cite{k02_JiangConrath}. It is defined as follows:
...
\subsection{Lin}
Lin's measure is the extension of Resnik's by taking the distance of how distant the terms are from their common ancestor into account\cite{k03_Lin}. It is defined as follows:
...
This method is proposed by Couto in 2003 \cite{k04_Couto}. In the matter of our work, if DO terms $t_1$ and $t_2$ are with an ascendant-descendant relationship, the semantic distance (inverse of similarity) between $t_1$ and $t_2$ is defined as follows:
...
Then we integrate the node depth and density factors in DO as conceptual distance factors to quantify the distance defined in equations \ref{eq:sim_dist} and \ref{eq:sim_dist2}. Considering there is an anscendant-descendant relationship between $t_0$ and $t_n$, and a path from ascendant node $t_0$ to descendant $t_n$ is $t_0,t_1,\dots,t_n$, where $n$ is the path length, the semantic distance between $t_0$ and $t_n$ is redefined as follows:
\begin{equation}\label{eq:sim_dist3}
 \Delta\left(t_0,t_n\right)=\sum_{i=0}^{n-1}D\left(t_i\right)\times E\left(t_i\right) \times \left(IC\left(t_{i+1}\right)-IC\left(t_{i}\right)\right)
\end{equation}
...
\end{equation}
where $d\left(t\right)$ denotes the depth of term $t$ in DO, and parameter $\alpha$ controls the contribution of depth factor in equation \ref{eq:sim_dist3} (In \Rpackage{DOSim}, we set it to 0.5). In the extreme case, like $\alpha $ is equal to 0, the contribution of depth factor becomes less significant, for $D\left(t\right)$ would be equal to 1.

$E\left(t\right)$ is defined as follows:
\begin{equation}\label{eq:et}
 E\left(t\right)=\left(1-\beta\right) \times \frac{\overline{E}}{e\left(t\right)} + \beta
\end{equation}
where $e\left(t\right)$ denotes the number of edges that start from $t$. $\overline{E}$ represents the average density of the DO system, which is equal to the ratio of the total number of edges in DO to the total number of terms in DO(it is equal to 2.85 in \Rpackage{DOSim}). Parameter $\beta$ controls the contribution of density factor in equation \ref{eq:sim_dist3} (In \Rpackage{DOSim}, we set it to 0.5).
...
In 2006, Schlicker et al. \cite{k06_Schlicker} developed the relevance similarity measure based on the measure proposed by Lin et al. mentioned above. The characteristic of this measure is that it used the probability of annotation of the MICA as a weighting factor to provide graph placement \cite{k07_Pesquita}. It can be formulated as follows:
...
\end{equation} where $p\left(t_{MICA}\right)=e^{-IC\left(t_{MICA)}\right)}$ in \Rpackage{DOSim} package.
...
GIC (Graph Information Content) is proposed by Presquita et al. \cite{k07_Pesquita} and it is an extension of graph-based similarity measures. The formula is shown as follows:
...
The measure of simIC (information coefficient similarity) is similar to the measure of relevance mentioned above. It could effectively integrate the information content and the structural information of terms in a DAG ontology \cite{k08_Li}. It is defined as follows:
...
\subsection{path}
This path-length measure is proposed by Wu et al.\cite{k09_Wu} in 1994, and it is defined as follows:
...
\subsection{lch}
The Leacock and Chodorow measure (lch) \cite{k10_Leacock} is computed as follows:
\begin{equation}\label{eq:lch}
 Sim_{lch}\left(t_1,t_2\right) = -\log\left(\frac{p}{2*depth}\right)
\end{equation}
where $p$ is the number of nodes on the shortest path between two terms $t_1$ and $t_2$ in DO and $depth$ is the maximum depth of the hierarchy (In \Rpackage{DOSim}, the maximum $depth$ of DO is 90).
...
\begin{equation}\label{eq:wang1}
 \left\{ \begin{array}{ll}
	  ...
	  S_A\left(t\right)=\max \left\{w_e \times S_A\left(t^{'}\right) \mid t^{'} \in childrenof\left(t\right) \right\} \quad if \  t\neq A
         \end{array} \right.
\end{equation}
where $w_e$ is the semantic contribution factor of edge $e$ ($e \in E_A$). It is set between 0 and 1 according to the type of relationship, e.g., "is-a" or "part-of". In DO, there is only one type of relationship, defined as "is-a", and we set $w_e$ to 0.7 in \Rpackage{DOSim}.

Then the senmantic similarity between two terms $A$ and $B$ is calculated as follows: 
...
\section{Measuring the Similarity between Human Genes}
...
\subsection{max}
This method is straight forward. Given two genes $g$ and $g^{'}$ annotated with DO terms $t_1,\dots,t_n$ and $t_{1}^{'},\dots,t_{m}^{'}$, the disease similarity between two genes $g$ and $g^{'}$ is defined as follows:
...
This method is similar to \textsl{funSimMax} except taking the average value between $rowScore$ and $columnScore$ as the measure, which can be formulateds as follows:
...
OA(optimal assignment) is proposed by Frohlich et al.\cite{k00_GOSim,k13_Froehlich}. Supposing that two gene $g$ and $g^{'}$ have DO terms annotations $t_1,\dots,t_n$ and $t_{1}^{'},\dots,t_{m}^{'}$ respectively, then a way of  calculating the similarity of $g$ and $g^{'}$ is to assign each term in the smaller of both lists to exactly one term in the longer one so that the sum of terms similarity is maximized. It can be state as follows: let $\pi$ be some permutation of either an $n$-subset of natural numbers $\{1,\dots,m\}$ or a $m$-subset of natural numbers $\{1,\dots,n\}$, then we are looking for the quantity:
...
\subsection{Wang}
It is proposed by Wang et.al \cite{k11_wang} using a best-match (best pairs technique) average combination strategy. Given two genes $g$ and $g^{'}$ annotated with DO terms $t_1,\dots,t_n$ ($DO_1$) and $t_{1}^{'},\dots,t_{m}^{'}$ ($DO_2$), we will find it is quite like the method \textsl{funSimAvg} as it is defined as follows:
...
\section{Geting Information of DO}
...
\Rpackage{DOSim} can do DO enrichment analysis for a list of Entrez Gene IDs by using \textbf{hypergeometric test} 
...
DOEnrichment(genelist,method = "fisher",filter=50,cutoff=0.01,adjustp="fdr")
